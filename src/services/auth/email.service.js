...
const sendResetPasswordEmail = async (to, token) => {
  ...
  const resetPasswordUrl = `${config.app_url}:${config.port}/reset-password?token=${token}`;
...
const sendVerificationEmail = async (to, token) => {
  ...
  const verificationEmailUrl = `${config.app_url}:${config.port}/v1/auth/verify-email?token=${token}`;
  //   const text = `Dear user,
